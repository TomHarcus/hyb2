...
sub initialize_hyb {
	my ( $self, $line, $exp ) = @_;
        chomp ( $line ); # added on 11.04.2019 to avoid problems with reading 15-column input
...
# returns 1 if two target sites overlap, 0 otherwise
	my ($self, $other) = @_;
	if ( $self->{_bit1_nm} eq $other->{_bit1_nm}     &&     $self->{_bit2_nm} eq $other->{_bit2_nm} ){
		my $bit1_ovlp = overlap( $self->{_bit1_st}, $self->{_bit1_end}, $other->{_bit1_st}, $other->{_bit1_end} );
		my $bit2_ovlp = overlap( $self->{_bit2_st}, $self->{_bit2_end}, $other->{_bit2_st}, $other->{_bit2_end} );
...
sub overlaps_reversed {
# returns 1 if two target sites overlap with the order of bits reversed, 0 otherwise
	my ($self, $other) = @_;
	if ( $self->{_bit1_nm} eq $other->{_bit2_nm}     &&     $self->{_bit2_nm} eq $other->{_bit1_nm} ){
		my $bit1_ovlp = overlap( $self->{_bit1_st}, $self->{_bit1_end}, $other->{_bit2_st}, $other->{_bit2_end} );
		my $bit2_ovlp = overlap( $self->{_bit2_st}, $self->{_bit2_end}, $other->{_bit1_st}, $other->{_bit1_end} );
		if ( $bit1_ovlp > 0   &&   $bit2_ovlp > 0 ){
			return 1;
		}
	}
...
sub merge_with_reversed {
# merges the current object with another one by extending the limits and increasing the count and averaging dG values
	my ($self, $other) = @_;
	$self->{_dG} = weighted_mean ( $self->{_dG}, $other->{_dG}, $self->{_count}, $other->{_count} );
	$self->{_count} += $other->{_count};
	$self->{_bit1_st} = min ( $self->{_bit1_st}, $other->{_bit2_st} );
	$self->{_bit1_end} = max ( $self->{_bit1_end}, $other->{_bit2_end} );
	$self->{_bit2_st} = min ( $self->{_bit2_st}, $other->{_bit1_st} );
	$self->{_bit2_end} = max ( $self->{_bit2_end}, $other->{_bit1_end} );
